...
export default function App() {
  ...
  const [clubeSelecionadoId, setClubeSelecionadoId] = useState<string>('1');
  ...
  const timeVisualizado = times.find(t => t.id === clubeSelecionadoId) || times[0];

  const idAtivoEfetivo = timeVisualizado ? timeVisualizado.id : clubeSelecionadoId;
      ...
      <header className="max-w-5xl w-full mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 pb-4 mb-6">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <button 
            ...
            className="flex-1 sm:flex-initial bg-green-500 hover:bg-green-600 text-slate-950 font-black px-5 py-2.5 rounded-lg transition-colors cursor-pointer text-sm disabled:opacity-30 shadow-lg shadow-green-500/10 disabled:cursor-not-allowed"
          >
              ...
              return (
                <div key={p.id} className="bg-slate-950 p-4 rounded-lg border border-slate-800 flex flex-col justify-between shadow-inner">
                  <div className="flex justify-between items-center text-sm font-bold">
                    <span className="text-slate-200 w-28 text-left truncate">{p.timeCasaNome}</span>
                    ...
                    <span className="text-slate-200 w-28 text-right truncate">{p.timeForaNome}</span>
                  </div>
          ...
          <div className="flex justify-between items-center border-b border-slate-800 pb-3 mb-4 flex-wrap gap-3">
            ...
            {abaAtiva === 'elenco' && times.length > 0 && (
              <select 
                ...
                onChange={(e) => setClubeSelecionadoId(e.target.value)}
                  ...
                  {[...times].sort((a,b) => b.pontos - a.pontos || (b.golsPro-b.golsContra) - (a.golsPro-a.golsContra) || b.golsPro - a.golsPro).map((time, idx) => {
                    ...
                    return (
                      <tr key={time.id} className="hover:bg-slate-900/40 transition-colors">
                        <td className="py-3 font-sans font-semibold text-slate-200 text-sm"><span className="text-slate-600 mr-2 font-mono text-xs">{idx + 1}.</span>{time.nome}</td>
